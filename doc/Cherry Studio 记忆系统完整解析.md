...
## 🔑 MCP Memory Server 触发的完整流程

### 配置层面（前提条件）

要让 MCP Memory Server 的知识图谱工具能被 LLM 调用，需要满足以下条件：
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  1️⃣ 全局设置：MCP Server 必须启用                              │
│     位置: 设置 → MCP 设置 → Memory Server → 开启 isActive      │
│                                                                 │
│  2️⃣ 助手设置：助手必须绑定该 MCP Server                        │
│     位置: 助手设置 → MCP → 勾选 Memory Server                   │
│                                                                 │
│  3️⃣ 工具调用支持：模型必须支持 Function Calling                │
│     检查: isPromptToolUse(assistant) || isSupportedToolUse     │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
```typescript
// 1. 用户发送消息后，fetchChatCompletion 被调用
// src/renderer/src/services/ApiService.ts:82-116

export async function fetchChatCompletion({ messages, assistant, ... }) {
  // ...

  // 2. 如果助手支持工具调用，获取 MCP 工具
  if (isPromptToolUse(assistant) || isSupportedToolUse(assistant)) {
    mcpTools.push(...(await fetchMcpTools(assistant)));
    // ↑ 这里会获取 Memory Server 的所有工具
  }
}

// 3. fetchMcpTools 函数获取助手绑定的 MCP 服务器的工具
// src/renderer/src/services/ApiService.ts:50-80

export async function fetchMcpTools(assistant: Assistant) {
  const allMcpServers = store.getState().mcp.servers;
  const activedMcpServers = allMcpServers.filter((s) => s.isActive);
  const assistantMcpServers = assistant.mcpServers || []; // 助手绑定的 MCP

  // 找到既激活又被助手绑定的 MCP 服务器
  ...
  // ↑ 这会调用 Memory Server 的 ListToolsRequestSchema
  // 返回: create_entities, create_relations, search_nodes 等工具
}

// 4. 工具被转换为 AI SDK 格式并传给 LLM
// src/renderer/src/aiCore/prepareParams/parameterBuilder.ts:125

let tools = setupToolsConfig(mcpTools);
// ↑ 转换 MCP 工具为 AI SDK Tool 格式

// 5. 构建最终请求参数
const params: StreamTextParams = {
  messages: sdkMessages,
  tools: tools, // 🔑 工具被添加到这里！
  // ...
};
```
...
│      {                                                          │
│        name: "search_nodes",                                    │
│        description: "Search nodes in memory",                   │
│        inputSchema: {...}                                       │
│      }                                                          │
│    ]                                                            │
│  }                                                              │
...
由于 LLM 的决策是自主的，你可以通过以下方式**增加触发概率**：

#### 方法 1：在系统提示词中明确指导

```typescript
// 在助手的 prompt 中添加
const systemPrompt = `你有权访问一个知识图谱记忆系统，包含以下工具：

- create_entities: 创建实体（人物、公司、概念等）
- create_relations: 创建实体间的关系
- add_observations: 为实体添加观察信息
- search_nodes: 搜索知识图谱

当用户提到以下信息时，请主动使用工具存储：
1. 人物信息（姓名、职位、关系）
2. 公司/组织信息
3. 项目关联

在回答问题前，如果涉及已知实体，请先搜索知识图谱。`;
```
...
```
用户: "请记住张三是ABC公司的产品经理"
       ↑ 明确的"记住"指令会提高 LLM 调用 create_entities 的概率

用户: "张三是做什么的？"
       ↑ 查询类问题会提高 LLM 调用 search_nodes 的概率
```
...
┌──────────┐          ┌──────────────┐         ┌─────────────┐
│ MCP 设置 │          │   助手设置   │         │  模型要求   │
│ Memory   │          │  绑定 Memory │         │ 支持 Tool   │
│ isActive │          │   Server     │         │ Calling     │
└────┬─────┘          └──────┬───────┘         └──────┬──────┘
     │                       │                        │
     └───────────────────────┼────────────────────────┘
                             │
              ...
              │      fetchMcpTools()         │
              │                              │
              │  1. 获取所有激活的 MCP Server │
              │  2. 过滤助手绑定的 Server     │
              │  3. 调用 window.api.mcp      │
              │     .listTools(memoryServer) │
              │  4. 返回工具列表             │
              └──────────────┬───────────────┘
                             │
                    返回 Memory Server 的工具:
                    • create_entities
                    • create_relations
                    • search_nodes
                    • add_observations
...
**触发 MCP Memory Server 的条件：**

1. **配置**：Memory Server 必须在 MCP 设置中**激活** (`isActive: true`)
2. **绑定**：助手必须在设置中**勾选** Memory Server
3. **模型**：使用的模型必须**支持 Tool Calling**
4. **用户消息**：发送一条消息，消息内容会影响 LLM 是否决定调用工具

**如何增加调用概率：**

- 在系统提示词中明确指导 LLM 使用这些工具
- 用户消息中使用明确的指令（如"记住"、"搜索"）

**核心真相：**
LLM 的决策是自主的，你**无法强制**它调用工具，只能通过提示词和消息内容**引导**它。
